...
const MemoriesScreen: React.FC<MemoriesScreenProps> = ({ highestUnlockedLevel }) => {
  const allMemories = [
    ...
    { title: 'Deep Core Resonance', date: 'CYCLE 189' },
    { title: 'Beta Subroutine', date: 'CYCLE 255' }
  ];
      ...
      <div className="flex-1 overflow-y-auto animated-scrollbar pb-24 pr-4 border-t border-theme-primary/20 pt-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {memories.map((mem, idx) => (
            <div key={idx} className="glass-panel border border-theme-primary/10 hover:border-theme-primary/50 p-2 cursor-pointer group flex flex-col transition-all hover:scale-[1.02]">
              <div className="w-full h-40 bg-zinc-900 border border-zinc-800 relative overflow-hidden mb-3">
                <div className="absolute inset-0 bg-theme-primary/5 z-10 group-hover:bg-transparent transition-colors"></div>
                ...
                <div className="w-full h-full filter grayscale sepia hue-rotate-[180deg] saturate-200 group-hover:grayscale-0 transition-all duration-500 flex items-center justify-center opacity-30 group-hover:opacity-100">
                   <div className="w-full h-full bg-[repeating-linear-gradient(45deg,transparent,transparent_10px,rgba(var(--theme-primary),0.1)_10px,rgba(var(--theme-primary),0.1)_20px)] animate-pulse-soft"></div>
                </div>
              ...
              <div className="px-2 pb-2">
                <h3 className="text-sm font-black font-headline uppercase text-white group-hover:text-theme-primary transition-colors tracking-widest">{mem.title}</h3>
